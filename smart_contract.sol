...
contract YoungBabyDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxWalletBalance = totalSupply().mul(3).div(100);
    uint256 public swapTokensAtAmount = 2 * 10**6 * 10**18;
 
    uint256 public liquidityFee = 2;
    uint256 public previousLiquidityFee;
    uint256 public dogeDividendRewardsFee = 7;
    uint256 public previousDogeDividendRewardsFee;
    uint256 public marketingFee = 5;
    uint256 public previousMarketingFee;
    uint256 public totalFees = dogeDividendRewardsFee.add(marketingFee).add(liquidityFee);
    ...
    constructor() ERC20("Young Baby Doge", "YBD") {
    	dogeDividendTracker = new DogeDividendTracker();
 
    	marketingWallet = 0xd6101B05C86B5a5Ffa66E8d789dD54B86737d1ee;
    ...
    receive() external payable {
  	...
  	function setWalletBalance(uint256 _maxWalletBalance) external onlyOwner{
        require(_maxWalletBalance>20 * 10**6 * 10**18);
  	    maxWalletBalance = _maxWalletBalance;
  	}
 
  	function setMaxBuyTransaction(uint256 _maxTxn) external onlyOwner {
        require(_maxTxn>20 * 10**6 * 10**18);
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "YBD: The marketing wallet is already this address");
    ...
    function setDogeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee.add(marketingFee).add(liquidityFee);
            dogeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dogeDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dogeDividendTracker), "YBD: The dividend tracker already has that address");
 
        DogeDividendTracker newDogeDividendTracker = DogeDividendTracker(payable(newAddress));
 
        require(newDogeDividendTracker.owner() == address(this), "YBD: The new dividend tracker must be owned by the YOUNG BABY DOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "YBD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "YBD: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "YBD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "YBD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "YBD: Cannot update gasForProcessing to same value");
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("YBD_Doge_Dividend_Tracker", "YBD_Doge_Dividend_Tracker", 0xbA2aE424d960c26247Dd6c32edC70B295c744C43) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "YBD_Doge_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "YBD_Doge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YOUNG BABY DOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "YBD_Doge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "YBD_Doge_Dividend_Tracker: Cannot update claimWait to same value");
